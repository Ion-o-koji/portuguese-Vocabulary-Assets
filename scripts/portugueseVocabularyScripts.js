...
<div id="fs-overlay" onclick="enterFsFromOverlay()">
  ...
  <div id="page-view" class="page">
    ...
    <div class="sort-row">
      ...
      <button class="sort-chip" id="sc-conf" onclick="toggleConfSort()" title="Sort by confidence level">🔴 Weakest</button>
      <select class="sort-cat-sel" id="view-conf-filter" onchange="lsSet('pv_view_conf',this.value);render()" title="Confidence level filter">
        <option value="all">All Levels</option>
        <option value="weak">🔴 Weak (Unknown + Unstudied)</option>
      ...
      <select class="sort-cat-sel" id="copy-pt-order" title="Copy order" style="min-width:104px">
        <option value="view">Current order</option>
        <option value="weak">Weakest first</option>
        <option value="strong">Strongest first</option>
      </select>
      <button class="bbtn" id="btn-copy-pt" onclick="copyPtColumn()" title="Copy the Portuguese column to the clipboard">📋 Copy PT</button>
    </div>
    <div class="twrap">
      <table id="tbl">
        <thead>
          <tr>
            ...
            <th class="col-ro" onclick="clickSort(2)">Pronunciation</th>
    ...
    <div class="form-wrap">
      ...
      <div class="form-card">
        <form id="add-form" onsubmit="submitAdd(event)">
          ...
          <div class="fg"><label>Pronunciation</label><input id="f-ro" type="text" placeholder="olá" autocomplete="off" /></div>
    ...
    <div class="trans-wrap">
      ...
      <div class="trans-content" id="dict-content">
        <div class="trans-landing">
          ...
          <h3>Brazilian Portuguese ⇆ English Dictionary</h3>
    ...
    <div class="study-wrap">
      ...
      <div class="study-controls">
        <div class="study-ctrl-row">
          <select id="study-filter" class="study-cat-sel active-sel study-multi-sel" multiple size="4" onchange="onStudyCatChange('user');initStudy()" title="Pick one or more categories"></select>
        ...
        <div class="study-ctrl-row">
          ...
          <select id="study-order" onchange="initStudy()" title="Deck order" style="min-width:120px">
        ...
        <div class="study-ctrl-row" style="justify-content:flex-end;gap:16px;padding-top:2px">
          ...
          <label class="study-opt-lbl"><span>Pronunciation</span><label class="tog" style="margin-left:6px"><input type="checkbox" id="opt-studyro" checked onchange="saveSettings()"><span class="tog-sl"></span></label></label>
        ...
        <div class="study-ctrl-row" style="justify-content:flex-end;gap:16px;padding-top:2px">
          <label class="study-opt-lbl" title="After every 5 rated cards, match them up"><span>Match 5</span><label class="tog" style="margin-left:6px"><input type="checkbox" id="opt-match5" onchange="saveSettings()"><span class="tog-sl"></span></label></label>
          <label class="study-opt-lbl" title="Favourited words show up more often"><span>⭐ Boost</span><label class="tog" style="margin-left:6px"><input type="checkbox" id="opt-favboost" onchange="saveSettings();initStudy()"><span class="tog-sl"></span></label></label>
        </div>
      </div>
      ...
      <div class="flashcard" id="flashcard" onclick="flipCard()">
        ...
        <button class="fc-star-btn" id="fc-star-btn" onclick="toggleCardFav(event)" title="Add to favourites">☆</button>
        <button class="fc-fs-btn" id="fc-fs-btn" onclick="toggleCardFullscreen(event)" title="Full screen flashcards">⛶</button>
      ...
      <div class="match-panel" id="match-panel" style="display:none">
        <div class="match-head">🧩 Quick Match — pair the last 5 cards</div>
        <div class="match-grid">
          <div class="match-col" id="match-col-pt"></div>
          <div class="match-col" id="match-col-en"></div>
        </div>
        <div class="match-status" id="match-status"></div>
        <button class="bbtn" id="match-skip" onclick="finishMatch(true)">Skip →</button>
      </div>
      </div>
  ...
  <div id="page-ref" class="page">
    <div class="ref-wrap">
      <h2>📚 Brazilian Portuguese Reference</h2>
      <div class="ref-section">
        <h3>Pronunciation Guide</h3>
        <div class="ref-phonetic-grid" id="ref-phonetic-grid"></div>
        <div class="ref-note">Tip: Tap any card below to hear it spoken in Brazilian Portuguese.</div>
      </div>
      <div class="ref-section">
        <h3>Diacritics Explained — Brazilian Portuguese</h3>
        <div class="ref-note">Accents never change a word’s meaning by themselves — they tell you which syllable to stress, whether a vowel is open or closed, and when a vowel is nasal. Tap a card to hear an example.</div>
        <div class="diac-list" id="ref-diacritics"></div>
      </div>
      <div class="ref-section">
        <h3>Numbers</h3>
        <div class="ref-num-grid" id="ref-numbers"></div>
        <div class="ref-note">Note: 1 and 2 change with gender (um/uma, dois/duas). From 200 up, hundreds also agree: duzentas mulheres, duzentos homens. Use "e" between parts: 1.234 = mil duzentos e trinta e quatro.</div>
      </div>
      <div class="ref-section">
        <h3>Conjugation Practice</h3>
        <div class="ref-note">Regular verb endings for the three Brazilian Portuguese verb families. Drop the -ar / -er / -ir from the infinitive and add the ending below.</div>
        <div class="conj-practice">
          <div class="conj-prompt" id="conj-prompt">Loading…</div>
          <input class="write-input" id="conj-input" type="text" placeholder="Type the ending…" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" onkeydown="if(event.key==='Enter')checkConj()">
          <div class="write-result" id="conj-result" style="display:none"></div>
          <div class="study-action-row">
            <button class="study-skip-btn" style="flex:1;font-size:.76rem" onclick="newConjQ()">Skip →</button>
            <button class="study-check-btn" id="conj-check-btn" onclick="checkConj()">✓ Check</button>
          </div>
        </div>
        <div class="conj-table-wrap">
          <table class="vowel-table conj-table">
            <thead>
              <tr>
                <th>Ending</th>
                <th>Tense</th>
  ...
  <div class="sovl" id="sovl" onclick="sovlBg(event)">
    <div class="esheet">
      ...
      <div class="sheet-body">
        ...
        <div class="fg"><label>Pronunciation</label><input id="e-ro" type="text" autocomplete="off" /></div>
  ...
  <div class="movl" id="cfg-ovl" onclick="if(event.target===this)closeCfg()">
    <div class="mcard" style="max-width:520px">
        ...
        <div class="cfg-row">
          <div class="cfg-row-left">
            ...
            <div class="col-toggle-grid" style="margin-top:6px">
              ...
              <label class="col-tog-lbl"><input type="checkbox" id="col-ro" checked onchange="applyColumns()">Pronunciation</label>
      ...
      <div class="cfg-section">
        <div class="cfg-section-title">Visible Tabs</div>
        <div style="font-size:.74rem;color:var(--dim);margin-bottom:10px">Turn a tab off to hide it from the navigation bar. At least one tab stays visible.</div>
        <div id="tab-visible-list" style="display:flex;flex-direction:column;gap:5px"></div>
        <div style="height:6px"></div>
        <button class="btn-g" style="font-size:.74rem;padding:7px 12px" onclick="resetHiddenTabs()">Show all tabs</button>
      </div>
      ...
      <div class="cfg-section">
        ...
        <div class="cfg-row">
          <div class="cfg-row-left">
            ...
            <div class="cr-sub">
              <div class="cr-sub">1. <a href="https://docs.google.com/document/d/1FzN0eHGZrG_TYfE0IWCOMA0Afci3b60qs2hopChJx74/edit?usp=drivesdk" target="_blank" rel="noopener" style="text-decoration:underline;color:#6b9bd1">Changelog</a></div>
              <div class="cr-sub">2. <a href="https://docs.google.com/document/d/1NIhfuWMh6D531enVAflE0z8HQPauZbJgjLjQNSFLn64/edit?usp=drivesdk" target="_blank" rel="noopener" style="text-decoration:underline;color:#6b9bd1">Feature Requests & Bug Reports</a></div>
            </div>
          </div>
    ...
    <div class="mcard">
      ...
      <div class="step-box">
        ...
        <div class="step-row">
          ...
          <div class="step-text">In <strong>Row 1</strong>, type these headers exactly in columns A through D:<br><code>English</code> &nbsp; <code>Portuguese</code> &nbsp; <code>Pronunciation</code> &nbsp; <code>Notes</code></div>
        </div>
        <div class="step-row">
  ...
  `;
    ...
    function speakPortuguese(text) {
      ...
      var src = 'https://translate.google.com/translate_tts?ie=UTF-8&q=' + encodeURIComponent(text) + '&tl=pt-BR&client=tw-ob&ttsspeed=0.9';
    ...
    function getStudyFilterValues() {
      ...
      if (!out.length) out = ['All'];
      return out;
    }

    function setStudyFilterValues(vals) {
      var sel = el('study-filter');
      if (!sel) return;
      var want = (vals && vals.length) ? vals : ['All'];
      var any = false;
      for (var i = 0; i < sel.options.length; i++) {
        var on = want.indexOf(sel.options[i].value) !== -1;
        sel.options[i].selected = on;
        if (on) any = true;
      }
      if (!any && sel.options.length) sel.options[0].selected = true;
    }

    function populateStudyFilter() {
      var sel = el('study-filter');
      if (!sel) return;
      var prev = getStudyFilterValues();
      sel.innerHTML = '';
    ...
    function refreshAllCatDropdowns() {
      var vcf = el('view-conf-filter');
      if (vcf && ls('pv_view_conf')) vcf.value = ls('pv_view_conf');
    ...
    var HIDDEN_TABS_KEY = 'pv_hidden_tabs';
    var hiddenTabs = safeJsonParse(ls(HIDDEN_TABS_KEY), []) || [];

    function isTabHidden(id) {
      return hiddenTabs.indexOf(id) !== -1;
    }

    function visibleTabs() {
      return tabOrder.filter(function(id) {
        return !isTabHidden(id);
      });
    }

    function applyHiddenTabs() {
      DEFAULT_TAB_ORDER.forEach(function(id) {
        var btn = el('nav-' + id);
        if (btn) btn.style.display = isTabHidden(id) ? 'none' : '';
      });
      var activeBtn = document.querySelector('.nav button.active');
      if (activeBtn && activeBtn.style.display === 'none') {
        var first = visibleTabs()[0];
        if (first) goPage(first, 'nav-' + first);
      }
    }

    function buildTabVisibleList() {
      var box = el('tab-visible-list');
      if (!box) return;
      box.innerHTML = '';
      tabOrder.forEach(function(id) {
        var navBtn = el('nav-' + id);
        var label = navBtn ? navBtn.textContent.trim() : id;
        ...
        span.style.cssText = 'flex:1;font-size:.82rem';
        ...
        cbx.type = 'checkbox';
        cbx.checked = !isTabHidden(id);
        (function(tid, box2) {
          cbx.addEventListener('change', function() {
            toggleTabVisible(tid, box2.checked);
          });
        })(id, cbx);
    ...
    function toggleTabVisible(id, visible) {
      var i = hiddenTabs.indexOf(id);
      if (visible) {
        if (i !== -1) hiddenTabs.splice(i, 1);
      } else if (i === -1) {
        if (visibleTabs().length <= 1) {
          toast('Keep at least one tab visible.', 'err');
          buildTabVisibleList();
          return;
        }
        hiddenTabs.push(id);
      }
      lsSet(HIDDEN_TABS_KEY, JSON.stringify(hiddenTabs));
      applyHiddenTabs();
      buildTabVisibleList();
    }

    function resetHiddenTabs() {
      hiddenTabs = [];
      lsSet(HIDDEN_TABS_KEY, '[]');
      applyHiddenTabs();
      buildTabVisibleList();
      toast('All tabs visible.', 'ok');
    }

    function safeDefaultPage() {
      var dp = S.defaultPage || 'add';
      if (isTabHidden(dp)) dp = visibleTabs()[0] || dp;
      return dp;
    }
    ...
    function loadSettings() {
      ...
      if (g('opt-match5')) g('opt-match5').checked = !!S.match5;
      if (g('opt-favboost')) g('opt-favboost').checked = !!S.favBoost;
    ...
    function saveSettings() {
      ...
      if (g('opt-match5')) S.match5 = g('opt-match5').checked;
      if (g('opt-favboost')) S.favBoost = g('opt-favboost').checked;
    ...
    function toggleConfSort() {
      quickSort = (quickSort === 'conf') ? 'confdesc' : 'conf';
      sortCol = -1;
      ...
      if (sc) {
        ...
        sc.textContent = quickSort === 'conf' ? '\uD83D\uDD34 Weakest' : '\uD83D\uDFE2 Strongest';
      }
      render();
    }

    function copyPtColumn() {
      var order = ((el('copy-pt-order') || {}).value) || 'view';
      var rows = getRows().slice();
      if (order === 'weak' || order === 'strong') {
        rows.sort(function(a, b) {
          var d = confRankVal(a) - confRankVal(b);
          if (order === 'strong') d = -d;
          if (d) return d;
          var ea = (a.english || '').toLowerCase(),
            eb = (b.english || '').toLowerCase();
          return ea < eb ? -1 : ea > eb ? 1 : 0;
        });
      }
      var list = rows.map(function(r) {
        return (r.portuguese || '').trim();
      }).filter(Boolean);
      if (!list.length) {
        toast('Nothing to copy.', 'inf');
        return;
      }
      var text = list.join('\n');

      function _done() {
        toast('Copied ' + list.length + ' Portuguese words \u2713', 'ok');
      }
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(_done).catch(function() {
          _copyFallback(text, _done);
        });
      } else _copyFallback(text, _done);
    }
    ...
    function _copyFallback(text, cb) {
      try {
        ...
        toast('Could not copy on this device.', 'err');
      }
    }
    ...
    function setQuickSort(mode) {
      ...
      var scc = el('sc-conf');
      if (scc && mode !== 'conf') scc.textContent = '\uD83D\uDD34 Weakest';
      render();
    }
    ...
    function confRankVal(r) {
      var c = confidence[wordKey(r)];
      return c === undefined ? 0.5 : c;
    }
    ...
    function getRows() {
      var q = ((el('q') || {}).value || '').trim().toLowerCase(),
        ...
        cfl = ((el('view-conf-filter') || {}).value) || 'all';
      ...
      if (cfl !== 'all') rows = rows.filter(function(r) {
        var c = confidence[wordKey(r)];
        if (cfl === 'got') return c === 2;
        if (cfl === 'kinda') return c === 1;
        if (cfl === 'unknown') return c === 0;
        if (cfl === 'unstudied') return c === undefined;
        if (cfl === 'weak') return c === 0 || c === undefined;
        return true;
      });
      if (q) rows = rows.filter(function(r) {
        return ((r.english || '') + (r.portuguese || '') + (r.romanization || '') + (r.notes || '') + (r.category || '')).toLowerCase().indexOf(q) !== -1;
      });
      if (quickSort === 'conf' || quickSort === 'confdesc') {
        rows.sort(function(a, b) {
          var d = confRankVal(a) - confRankVal(b);
          if (quickSort === 'confdesc') d = -d;
          if (d) return d;
          var ea = (a.english || '').toLowerCase(),
            eb = (b.english || '').toLowerCase();
          return ea < eb ? -1 : ea > eb ? 1 : 0;
    ...
    function initStudy() {
      ...
      if (presetFilter === '__weak__') pool = pool.filter(function(r) {
        var c = confidence[wordKey(r)];
        return c === 0 || c === undefined;
      });
      else if (presetFilter === '__due__') pool = pool.filter(function(r) {
        return isSrsDue(wordKey(r));
      });
      else if (presetFilter === '__nums__') {
      ...
      } else if (presetFilter === '__diac__') {
        var _diac = window._refDiac || [];
        pool = _diac.map(function(d) {
          return {
            english: d.name,
            portuguese: d.mark,
            romanization: d.name,
            notes: (d.what || '') + ' \u2014 e.g. ' + (d.ex || ''),
            category: 'Diacritics',
            id: 'ref_' + d.mark
          };
        });
      } else if (presetFilter.indexOf('set:') === 0) {
        var sid = presetFilter.slice(4),
          sset = studySets.find(function(st) {
            return st.id === sid;
          });
      ...
      } else if (selVals.indexOf('All') === -1) {
        pool = pool.filter(function(r) {
          var cat = r.category || '';
          for (var i = 0; i < selVals.length; i++) {
            var v = selVals[i];
            if (v === '__fav__') {
              if (isFav(r)) return true;
            } else if (v === '__nosent__') {
              if (cat !== 'Sentences') return true;
            } else if (v === cat) return true;
          }
          return false;
        });
      }
      if (orderMode === 'weak' || orderMode === 'strong') {
        pool.sort(function(a, b) {
          var d = confRankVal(a) - confRankVal(b);
          if (orderMode === 'strong') d = -d;
          if (d) return d;
          return Math.random() - .5;
        });
      } else if (S.srs) {
    ...
    function updateCardStar() {
      var b = el('fc-star-btn');
      if (!b) return;
      var r = studyDeck[studyIdx];
      var on = r ? isFav(r) : false;
      b.textContent = on ? '\u2B50' : '\u2606';
      b.classList.toggle('on', !!on);
      b.title = on ? 'Remove from favourites' : 'Add to favourites';
    }

    function toggleCardFav(e) {
      if (e && e.stopPropagation) e.stopPropagation();
      var r = studyDeck[studyIdx];
      if (!r) return;
      toggleFav(r);
      updateCardStar();
      toast(isFav(r) ? 'Added to favourites \u2B50' : 'Removed from favourites', 'ok');
    }

    function toggleCardFullscreen(e) {
      if (e && e.stopPropagation) e.stopPropagation();
      var st = el('fc-stage');
      if (!st) return;
      if (!document.fullscreenElement && !document.webkitFullscreenElement) {
        ...
        } else toast('Full screen not supported here.', 'inf');
      } else {
    ...
    function _updateCardFsBtn() {
      var st = el('fc-stage'),
        b = el('fc-fs-btn');
      var fsEl = document.fullscreenElement || document.webkitFullscreenElement;
      var on = !!(st && fsEl === st);
      if (st) st.classList.toggle('fc-stage-fs', on);
      if (b) {
        b.textContent = on ? '\u2715' : '\u26F6';
        b.title = on ? 'Exit full screen' : 'Full screen flashcards';
      }
    }
    ...
    function showMatchPanel() {
      var panel = el('match-panel');
      var cards = matchBuffer.slice(-5);
      if (!panel || cards.length < 2) {
        hideMatchPanel();
        return false;
      }
      matchActive = true;
      ...
      var ptCol = el('match-col-pt'),
        enCol = el('match-col-en');
      if (!ptCol || !enCol) {
        hideMatchPanel();
        return false;
      }
      ...
      function mk(col, r, side) {
        var b = document.createElement('button');
        b.className = 'match-btn' + (side === 'pt' ? ' match-pt pt' : '');
        b.textContent = side === 'pt' ? (r.portuguese || '?') : (r.english || '?');
        b.setAttribute('data-mk', wordKey(r));
        b.addEventListener('click', function() {
          onMatchPick(b, side, total);
        });
        col.appendChild(b);
      }
      cards.slice().sort(function() {
        return Math.random() - .5;
      }).forEach(function(r) {
        mk(ptCol, r, 'pt');
      });
      ...
      var st = el('match-status');
      if (st) st.textContent = '0 / ' + total + ' matched';
      return true;
    }

    function onMatchPick(btn, side, total) {
      if (!btn || btn.classList.contains('matched')) return;
      if (matchPick && matchPick.btn === btn) {
        btn.classList.remove('sel');
        matchPick = null;
        return;
      }
      if (matchPick && matchPick.side === side) {
        matchPick.btn.classList.remove('sel');
        matchPick = {
          btn: btn,
          side: side
        };
      ...
      if (a.getAttribute('data-mk') === b.getAttribute('data-mk')) {
        a.classList.add('matched');
        b.classList.add('matched');
        matchDone++;
        var st = el('match-status');
        if (st) st.textContent = matchDone + ' / ' + total + ' matched';
        if (matchDone >= total) setTimeout(function() {
          finishMatch();
        }, 500);
      } else {
        a.classList.add('bad');
    ...
    function finishMatch(skipped) {
      ...
      if (fc) fc.style.display = studyDeck.length === 0 ? 'none' : 'flex';
      if (fsb) fsb.style.display = studyDeck.length > 0 ? 'block' : 'none';
      if (!skipped) toast('Nice \u2014 all matched \u2713', 'ok');
      nextCard();
    }
    ...
    function rateCard(rating) {
      ...
      if (S.match5) {
        var mk2 = wordKey(r);
        var seen = matchBuffer.some(function(x) {
          return wordKey(x) === mk2;
        });
        if (!seen && (r.portuguese || '') && (r.english || '')) matchBuffer.push(r);
      }
      nextCard();
    }
    ...
    function nextCard() {
      ...
      if (S.match5 && !matchActive && matchBuffer.length >= 5) {
        if (showMatchPanel()) return;
      }
    ...
    function buildReferencePages() {
        ...
        { pt: 'duzentos / duzentas', word: '', n: '200' },
        { pt: 'trezentos / trezentas', word: '', n: '300' },
        { pt: 'quatrocentos', word: '', n: '400' },
        { pt: 'quinhentos', word: '', n: '500' },
        { pt: 'seiscentos', word: '', n: '600' },
        { pt: 'setecentos', word: '', n: '700' },
        { pt: 'oitocentos', word: '', n: '800' },
        { pt: 'novecentos', word: '', n: '900' },
        { pt: 'mil', word: '', n: '1.000' },
        { pt: 'dois mil', word: '', n: '2.000' },
        { pt: 'dez mil', word: '', n: '10.000' },
        { pt: 'cem mil', word: '', n: '100.000' },
        { pt: 'um milh\u00E3o', word: '', n: '1.000.000' },
        { pt: 'dois milh\u00F5es', word: '', n: '2.000.000' },
        { pt: 'um bilh\u00E3o', word: '', n: '1.000.000.000' }
      ];
      var diacritics = [
        { mark: '\u00E1 \u00E9 \u00ED \u00F3 \u00FA', name: 'Acute (acento agudo)', what: 'Marks the stressed syllable and makes the vowel open.', ex: 'caf\u00E9, \u00E1gua, a\u00ED, av\u00F3, \u00FAltimo', speak: 'caf\u00E9' },
        { mark: '\u00E2 \u00EA \u00F4', name: 'Circumflex (acento circunflexo)', what: 'Marks the stressed syllable and makes the vowel closed (tighter, higher).', ex: 'p\u00EA-ssego, av\u00F4, c\u00E2mera', speak: 'av\u00F4' },
        { mark: '\u00E3 \u00F5', name: 'Tilde (til)', what: 'Nasalises the vowel \u2014 air goes through the nose. Very common in Brazil.', ex: 'p\u00E3o, m\u00E3e, irm\u00E3o, li\u00E7\u00F5es', speak: 'p\u00E3o' },
        { mark: '\u00E0', name: 'Grave (acento grave)', what: 'Only used for the crasis: the preposition "a" merged with the article "a".', ex: 'Vou \u00E0 praia (a + a)', speak: 'Vou \u00E0 praia' },
        { mark: '\u00E7', name: 'Cedilla (cedilha)', what: 'Makes "c" sound like a sharp "s" before a, o, u. Never used before e or i.', ex: 'a\u00E7\u00FAcar, cora\u00E7\u00E3o, come\u00E7ar', speak: 'cora\u00E7\u00E3o' },
        { mark: '\u00FC / \u00E4 (retired)', name: 'Diaeresis (trema)', what: 'Removed by the 2009 spelling reform \u2014 you will only see it in old texts and foreign names.', ex: 'lingui\u00E7a (formerly ling\u00FCi\u00E7a)', speak: 'lingui\u00E7a' },
        { mark: 'Stress rule', name: 'When no accent appears', what: 'Words ending in a, e, o, em, ens stress the second-to-last syllable; words ending in i, u, l, r, z, im, um, \u00E3o stress the last one. An accent is written whenever a word breaks that pattern.', ex: 'casa (CA-sa) vs. papel (pa-PEL) vs. l\u00E1pis (L\u00C1-pis)', speak: 'l\u00E1pis' }
      ];
      var conjRows = [
        ['-ar', 'Present', 'o', 'a', 'amos', 'am', '-'],
        ['-ar', 'Preterite', 'ei', 'ou', 'amos', 'aram', '-'],
        ['-ar', 'Imperfect', 'ava', 'ava', '\u00E1vamos', 'avam', '-'],
        ['-ar', 'Future', 'arei', 'ar\u00E1', 'aremos', 'ar\u00E3o', '-'],
        ['-ar', 'Conditional', 'aria', 'aria', 'ar\u00EDamos', 'ariam', '-'],
        ['-ar', 'Pres Subj', 'e', 'e', 'emos', 'em', '-'],
        ['-ar', 'Imp Subj', 'asse', 'asse', '\u00E1ssemos', 'assem', '-'],
        ['-ar', 'Imperative', '-', 'e', 'emos', 'em', 'voc\u00EA'],
        ['-ar', 'Gerund', '-', '-', '-', '-', 'ando'],
        ['-ar', 'Participle', '-', '-', '-', '-', 'ado'],
        ['-er', 'Present', 'o', 'e', 'emos', 'em', '-'],
        ['-er', 'Preterite', 'i', 'eu', 'emos', 'eram', '-'],
        ['-er', 'Imperfect', 'ia', 'ia', '\u00EDamos', 'iam', '-'],
        ['-er', 'Future', 'erei', 'er\u00E1', 'eremos', 'er\u00E3o', '-'],
        ['-er', 'Conditional', 'eria', 'eria', 'er\u00EDamos', 'eriam', '-'],
        ['-er', 'Pres Subj', 'a', 'a', 'amos', 'am', '-'],
        ['-er', 'Imp Subj', 'esse', 'esse', '\u00EAssemos', 'essem', '-'],
        ['-er', 'Imperative', '-', 'a', 'amos', 'am', 'voc\u00EA'],
        ['-er', 'Gerund', '-', '-', '-', '-', 'endo'],
        ['-er', 'Participle', '-', '-', '-', '-', 'ido'],
        ['-ir', 'Present', 'o', 'e', 'imos', 'em', '-'],
        ['-ir', 'Preterite', 'i', 'iu', 'imos', 'iram', '-'],
        ['-ir', 'Imperfect', 'ia', 'ia', '\u00EDamos', 'iam', '-'],
        ['-ir', 'Future', 'irei', 'ir\u00E1', 'iremos', 'ir\u00E3o', '-'],
        ['-ir', 'Conditional', 'iria', 'iria', '\u00EDamos', 'iriam', '-'],
        ['-ir', 'Pres Subj', 'a', 'a', 'amos', 'am', '-'],
        ['-ir', 'Imp Subj', 'isse', 'isse', '\u00EDssemos', 'issem', '-'],
        ['-ir', 'Imperative', '-', 'a', 'amos', 'am', 'voc\u00EA'],
        ['-ir', 'Gerund', '-', '-', '-', '-', 'indo'],
        ['-ir', 'Participle', '-', '-', '-', '-', 'ido']
      ];
            var phrases = [
      ...
      window._refDiac = diacritics;
      window._conjRows = conjRows;
      var dl = el('ref-diacritics');
      if (dl) {
        dl.innerHTML = '';
        diacritics.forEach(function(d) {
          var card = document.createElement('div');
          card.className = 'diac-card';
          card.innerHTML = '<div class="diac-mark pt">' + esc(d.mark) + '</div>' +
            '<div class="diac-body"><div class="diac-name">' + esc(d.name) + '</div>' +
            '<div class="diac-what">' + esc(d.what) + '</div>' +
            '<div class="diac-ex pt">' + esc(d.ex) + '</div></div>';
          (function(txt) {
            card.addEventListener('click', function() {
              if (txt) speakPortuguese(txt);
            });
          })(d.speak);
          card.title = 'Tap to hear an example';
          dl.appendChild(card);
        });
      }
      var ctb = el('conj-table-body');
      if (ctb) {
        ctb.innerHTML = '';
        conjRows.forEach(function(row) {
          var tr = document.createElement('tr');
          tr.innerHTML = '<td class="vt-ro">' + esc(row[0]) + '</td><td class="vt-ro">' + esc(row[1]) + '</td>' +
            '<td class="vt-pt pt">' + esc(row[2]) + '</td><td class="vt-pt pt">' + esc(row[3]) + '</td>' +
            '<td class="vt-pt pt">' + esc(row[4]) + '</td><td class="vt-pt pt">' + esc(row[5]) + '</td>' +
            '<td class="vt-ro">' + esc(row[6]) + '</td>';
          ctb.appendChild(tr);
        });
      }
      newConjQ();
    ...
    var CONJ_PERSONS = [
      ...
      { i: 3, label: 'ele/voc\u00EA' },
      { i: 4, label: 'n\u00F3s' },
      { i: 5, label: 'eles/voc\u00EAs' }
    ];

    function newConjQ() {
      var rows = window._conjRows || [];
      ...
      do {
        ...
        var tense = row[1];
        if (tense === 'Gerund' || tense === 'Participle') {
          person = { i: 6, label: 'form' };
        } else {
          var cands = CONJ_PERSONS.filter(function(pp) {
            return row[pp.i] && row[pp.i] !== '-';
          });
          person = cands[Math.floor(Math.random() * cands.length)];
        }
        answer = person ? row[person.i] : '-';
        tries++;
      } while ((!answer || answer === '-') && tries < 40);
      ...
      var pr = el('conj-prompt');
      if (pr) {
        pr.innerHTML = '<span class="conj-q-end pt">' + esc(row[0]) + '</span> \u2014 <span class="conj-q-tense">' + esc(row[1]) +
          '</span>' + (person.i === 6 ? '' : ' \u2014 <span class="conj-q-person pt">' + esc(person.label) + '</span>');
      }
      var inp = el('conj-input');
      if (inp) {
        inp.value = '';
        inp.disabled = false;
      }
      var res = el('conj-result');
      if (res) {
        res.style.display = 'none';
        res.textContent = '';
      }
      var btn = el('conj-check-btn');
      if (btn) btn.textContent = '\u2713 Check';
      _conjChecked = false;
    }

    var _conjChecked = false;
    ...
    function checkConj() {
      ...
      var inp = el('conj-input'),
        res = el('conj-result');
      if (!inp || !res) return;
      var m = answerMatch(inp.value, _conjQ.answer);
      res.style.display = 'block';
      if (m === 'exact') {
        res.className = 'write-result correct';
        res.textContent = '\u2713 Correct! ' + _conjQ.answer;
      } else if (m === 'accent') {
        res.className = 'write-result correct';
        res.textContent = '\u2713 Correct \u2014 mind the accents: ' + _conjQ.answer;
      } else {
        res.className = 'write-result wrong';
        res.textContent = '\u2717 Answer: ' + _conjQ.answer;
      }
      _conjChecked = true;
      var btn = el('conj-check-btn');
    ...
    function dictLangFor(api, code) {
      if (code !== PT_CODE) return code;
      if (api === 'lingva' || api === 'libre') return 'pt';
      return PT_CODE;
    }

    async function dictFetchAll(word) {
      var from = dictIsPortuguese(word) ? PT_CODE : 'en',
        to = from === PT_CODE ? 'en' : PT_CODE;
      ...
      var apis = [{
        ...
        {
          fn: function() {
            return dictFetchApi3(word, dictLangFor('lingva', from), dictLangFor('lingva', to));
          },
          name: 'API3'
        },
        {
          fn: function() {
            return dictFetchApi4(word, dictLangFor('libre', from), dictLangFor('libre', to));
          },
          name: 'API4'
        },
        {
    ...
    function dictRenderResults() {
      ...
      var html = '<div class="trans-entry-header"><div><div class="trans-word">' + esc(query) + '</div><div class="trans-detected">Detected: ' + (dictState.detectedLang !== 'en' ? 'Portuguese 🇧🇷' : 'English 🇬🇧') + '</div></div><div class="trans-result-count">' + ok + ' / 5 APIs</div></div>';
    ...
    function dictRenderSummary(results, query) {
      ...
      uniqDefs.slice(0, 10).forEach(function(def) {
        ...
        var _en = dictState.detectedLang !== 'en' ? dictClean(def.text) : query;
        var _pt = dictState.detectedLang !== 'en' ? query : dictClean(def.text);
        _dictAddItems.push({
          en: _en,
          pt: _pt
        });
      ...
      success.forEach(function(x) {
        ...
        if (apiData.results?.length) {
          ...
          apiData.results.slice(0, 4).forEach(function(r) {
            ...
            if (c) {
              var _ai = _dictAddItems.length;
              var _en = dictState.detectedLang !== 'en' ? c : query;
              var _pt = dictState.detectedLang !== 'en' ? query : c;
              _dictAddItems.push({
                en: _en,
                pt: _pt
              });
        ...
        setTimeout(function() {
          ...
          setTimeout(function() {
            try {
              ...
              var roIdx = fc('pronunciation');
              if (roIdx === -1) roIdx = fc('romanization');
              var noIdx = fc('notes'),
                caIdx = fc('category'),
                dtIdx = fc('date added');
              var _p = function(x) {
    ...
    async function exportCSV() {
      ...
      var lines = [
        ['English', 'Portuguese', 'Pronunciation', 'Notes', 'Category', 'Date Added'].join(',')
      ];
      allRows.forEach(function(r) {
    ...
    function checkWriting() {
      ...
      if (!correctRaw) {
        ...
        var m = answerMatch(ans, correctRaw);
        if (m === 'exact') {
          res.className = 'write-result correct';
          res.textContent = '\u2713 Correct! ' + correctRaw;
        } else if (m === 'accent') {
          res.className = 'write-result correct';
          res.textContent = '\u2713 Correct \u2014 mind the accents: ' + correctRaw;
        } else {
          res.className = 'write-result wrong';
    ...
    function answerMatch(ans, correct) {
      var a = normAnswer(ans),
        c = normAnswer(correct);
      if (!a) return 'no';
      if (a === c) return 'exact';
      if (stripAccents(a) === stripAccents(c)) return 'accent';
      return 'no';
    }
    ...
    function checkListening() {
      ...
      if (!correctRaw) {
        ...
        var m2 = answerMatch(ans, correctRaw);
        if (m2 === 'exact') {
          res.className = 'write-result correct';
          res.textContent = '\u2713 Correct! ' + correctRaw;
        } else if (m2 === 'accent') {
